...
const SubscriptionTypeDropdDownButton = ({ subType, refreshData }) => {
  const { encodedToken } = useEncodedToken();

  const handleToggleActivation = () => {
    Api.activateDeactivateSubType(subType.id, encodedToken).then((response) => {
      if (response.ok) {
        console.log("Subtype deactivated successfully");
        refreshData();
      } else {
        console.error("Error");
      }
  ...
  const handleEdit = () => {
    navigate(`/editSubscriptionType/${subType.id}`);
  };
  ...
  return (
    <div className="flex flex-row gap">
      ...
      <div>
        {subType.subscriptionTypeStatusEnum == "ACTIVATED" ? (
          <button
...
export default SubscriptionTypeDropdDownButton;
