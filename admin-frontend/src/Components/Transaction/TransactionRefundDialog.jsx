...
export default function TransactionRefundDialog({handleClose, open, transaction, refreshTransactions}) {
    ...
    const handleRefund = () => {
        if (refund?.confirm === "YES" && !submitted) {
          setSubmitted(true);
          console.log({...refund, amt : transaction?.totalAmount, transactionId : transaction?.id})
          Api.refund({...refund, amt : transaction?.totalAmount, transactionId : transaction?.id}, encodedToken)
          .then(res => {
            if (res.ok) {
    ...
    <DialogContent>
      <DialogContentText>
          <span className="text-xl">{"Payee Username : " + transaction?.receiver}</span>

          <br></br>
          <span className="text-xl">{"Refund Amount : " + transaction?.totalAmount}</span>
      </DialogContentText>
      {fields.map(field => {
        return (
            <div>
