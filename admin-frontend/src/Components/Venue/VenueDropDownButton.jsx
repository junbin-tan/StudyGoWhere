...
const VenueDropDownButton = ({ venue, refreshData }) => {
  const { encodedToken } = useEncodedToken();

  const handleToggleActivation = () => {
    Api.banUnbanVenue(venue.id, encodedToken).then((response) => {
      if (response.ok) {
        console.log("Venue banned successfully");
        refreshData();
      } else {
        console.error("Error");
      }
...
export default VenueDropDownButton;
