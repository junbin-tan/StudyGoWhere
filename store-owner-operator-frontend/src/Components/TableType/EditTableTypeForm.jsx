...
const EditTableTypeForm = ({ tableTypeList }) => {
  ...
  useEffect(() => {
    Api.getTableTypeById(tableTypeId, encodedToken)
      .then((response) => {
        if (response.ok) {
      ...
      .then((data) => {
        console.log("Table type fetched successfully");
        console.log(data);
        setTableTypeDetail(data);
      })
      .catch((error) => {
        console.error("Error:", error.message);
      });
  }, [tableTypeId, encodedToken]);
  ...
  const handleEdit = async () => {
    try {
      const response = await Api.editTableType(tableTypeDetail, encodedToken);
      console.log(response);
      setOpenSnackbar(true);
      setSnackbarMessage("Table Type Edited Successfully");
    } catch (error) {
      console.error("An error occurred while editing table type:", error);
    }
  };
    ...
    <BodyCard>
      <div className="flex flex-col gap-8">
        <div className="flex flex-col">
          <h2 className="text-lg font-semibold">Edit Table Type Details</h2>
          <FieldInfo>Edit Table Type Details</FieldInfo>
        </div>
        <div className="flex flex-col">
...
export default EditTableTypeForm;
